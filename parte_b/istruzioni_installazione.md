...
1. Aprire il prompt dei comandi posizionandosi nella cartella principale del progetto.
2. Avviare lo script di installazione specifico per la piattaforma d'uso:
    <ol type="a">
    <li>Windows eseguire: <code>.\install.bat</code> utilizzando il prompt dei comandi</li>
...
2. Copiare la cartella `lido` appena generata dallo script nella cartella `lido_deploy/` all'interno della cartella `webapps`.
3. Avviare il server Tomcat tramite lo script `startup.sh` o `startup.bat` (windows) che si trova nella cartella di tomcat (es. `/usr/local/tomcat/bin/startup.sh` su sistemi Unix/Linux o `C:\Program Files\Apache Software Foundation\Tomcat 11.0\bin\startup.bat` su sistemi Windows).
...
1. **Configurazione preliminare delle credenziali:** l'accesso al manager richiede la configurazione di un utente abilitato. Per farlo aprire il file `conf/tomcat-users.xml` nella cartella di tomcat e aggiungere all'interno del tag principale la riga `<user username="admin" password = "TUA PASSWORD" roles="manager-gui>`
2. **Accesso alla dashboard:** aprire il browser e digitare `http://localhost:PORTA/manager/html`, inserendo le credenziali appena configurate 
...
| ------------------------------------------ | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| **Conflitto di Porta MySQL**               | Il db non risponde sulla porta standard o l'applicazione fallisce la connessione JDBC | - Premi i tasti `Win + R`, scrivi `services.msc` e premi invio.<br><br>- Cerca il servizio di MySQL (ad esempio `MySQL80`), fai clic con il tasto destro e apri le **Proprietà**.<br><br>- Leggi il percorso del file di configurazione (es. `my.ini`).<br><br>- Apri quel file con un editor di testo (come il Blocco note) e cerca la riga con `port=`.<br><br>- ripartire dal punto 3 inserendo la porta corretta |     |
...
| **Incompatibilità di versione JDK/Tomcat** | Tentativo di eseguire classi non compatibili tra di loro                              | Verificare che la versione del JDK impostata nelle variabili di ambiente corrisponda effettivamente alla versione di Tomcat utilizzata per il deploy (JDK 8 - Tomcat 9 oppure JDK 21 - Tomcat 11). In caso non lo fosse modificare la voce `JAVA_HOME` (es. `C:\Program Files\Java\jdk-21.0.11\`) e `path` togliendo la voce vecchia di java e aggiungendo questa                                                    |     |
